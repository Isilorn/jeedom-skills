...
## 6. Top-5 gotchas critiques

1. **`trigger` = mot réservé MySQL.** Toute requête sur `scenario.trigger` doit utiliser des backticks : `` `trigger` ``. Sans backticks → erreur SQL ou résultat silencieusement faux.
...
| `agenda` | `references/plugin-agenda.md` |
...
| `references/audit-templates.md` | Templates de rapport WF1 (12 sections fixes) |
...
| `references/plugin-agenda.md` | Plugin Agenda : événements, calendriers |
| `references/plugin-script.md` | Plugin Script : types, exécution |
| `references/plugin-generic-pattern.md` | Pattern d'inspection générique (tous autres plugins) |
...
| `scripts/scenario_tree_walker.py` | Parcours récursif scénario (anti-cycle, max_depth) |
